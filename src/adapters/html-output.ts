...
export function renderBlankPicklistHtml(opts?: { rowCount?: number; title?: string }): string {
  const rows = opts?.rowCount ?? 20;
  ...
  const rowHtml = Array.from({ length: rows }, (_, i) => {
    const bg = i % 2 === 1 ? ' style="background:#f2f2f2"' : '';
    return `<tr${bg}>
    <td class="num">${i + 1}</td>
    <td class="loc"></td>
    <td></td>
...
<section class="page">
  <h1>${title}</h1>
  <div class="meta">
    <span><b>Printed:</b> ${timestamp}</span>
  </div>
  <table>
    <thead><tr>
