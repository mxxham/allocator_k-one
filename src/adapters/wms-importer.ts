 ...
 * steps — validateImport() produces a preview report, executeImport() posts
 * the snapshot through the initial_import RPC in one database transaction.
...
import { loadWorkbook, type LoadedData } from './excel-input.js';
...
export function validateImport(sourceFile: string, loaded: LoadedData): ImportPreview {
  ...
  // Row-level sanity (loadWorkbook already dropped qty<=0 and missing
  // sku/expiry rows into warnings — surface anything it flagged as ERROR).
